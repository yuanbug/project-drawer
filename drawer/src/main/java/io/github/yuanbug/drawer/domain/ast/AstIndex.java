...
public class AstIndex {
    ...
    public boolean isAssignable(ResolvedTypeDeclaration expectedSuperType, String expectedSubTypeName) {
        ...
        if (expectedSuperType instanceof ResolvedReferenceTypeDeclaration declaration) {
            var sub = trySolveReferenceTypeDeclaration(expectedSubTypeName).orElse(null);
            if (null != sub) {
                return declaration.isAssignableBy(sub);
            }
