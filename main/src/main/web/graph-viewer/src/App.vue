...
function getData<T>(url: string, name: string, callback: (data: T) => void) {
...
const loadMethodLink = (method: MethodListItem) => {
  getData<MethodLink>(`api/method-info/method-link?methodId=${encodeURIComponent(method.methodId)}`, '获取方法关系链', data => (graphMethodLink.value = data))
}
</script>
